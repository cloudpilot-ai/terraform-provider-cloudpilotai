...
variable "wa_storage_class" {
  description = "StorageClass for VictoriaMetrics persistent volume. Required if the cluster has no default StorageClass (e.g. 'gp2', 'gp3'). Run `kubectl get storageclass` to find available options. Empty string uses cluster default."
  type        = string
  default     = ""
}
